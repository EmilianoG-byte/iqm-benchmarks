...
def randpsd(n, normalized="True", seed:int = 42):
    ...
    """
    dim = int(np.sqrt(n))
    H = randHerm(dim, seed=seed)
    U, _, _ = np.linalg.svd(H)
    rng = np.random.default_rng(seed+1)
    evals = rng.random(dim)
    if normalized == "True":
        evals /= np.sum(evals)
...
def random_gs(d, r, rK, n_povm, seeds:list[int]|None = None):
    ...
    seed: list of seeds used to generate the Kraus operators, POVM and initial state. If None, default seeds are used: [42, 43, 44].
    ...
    dim = int(np.sqrt(r))
    K = randKrausSet(d, dim, rK, seed=seed_kraus).copy()
    X = np.einsum("ijkl,ijnm -> iknlm", K, K.conj()).reshape((d, r, r))
    rho = randpsd(r, seed=seed_state).copy()
    A = randKrausSet(1, dim, n_povm, seed=seed_povm)[0].conj()
...
def perturbed_target_init(X_target, rK, seed:int = 42):
    ...
    pdim = int(np.sqrt(r))
    K_perturb = randKrausSet(d, pdim, rK, a=0.1, seed=seed)
